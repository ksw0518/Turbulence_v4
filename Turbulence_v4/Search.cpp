...
static inline int Negamax(Board& board, int depth, int alpha, int beta, bool doNMP, bool cutnode, ThreadData& data, Move excludedMove = NULLMOVE)
{
	...
	for (int i = 0; i < moveList.count; ++i)
	{
		...
		if (alpha >= beta)
		{
			...
			int historyDepth = depth + (bestValue > beta + 50);

			if ((move.Type & capture) == 0)
			{
				data.killerMoves[0][data.ply] = move;
				int mainHistBonus = std::min(2400, HISTORY_BASE + 420 * historyDepth);
				int contHistBonus = std::min(2400, CONTHIST_BASE + 290 * historyDepth);
				...
				int captHistBonus = std::min(2400, CAPTHIST_BASE + 400 * historyDepth);
