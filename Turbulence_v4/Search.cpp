...
int RookPawn_Corrhist[2][CORRHIST_SIZE];
...
void update_RookPawn_Corrhist(Board& board, const int depth, const int diff)
{
	uint64_t rookPawnKey = generate_RookPawn_Hash(board);
	int& entry = RookPawn_Corrhist[board.side][rookPawnKey % CORRHIST_SIZE];
	const int scaledDiff = diff * CORRHIST_GRAIN;
	const int newWeight = std::min(depth + 1, 16);
	entry = (entry * (CORRHIST_WEIGHT_SCALE - newWeight) + scaledDiff * newWeight) / CORRHIST_WEIGHT_SCALE;
	entry = std::clamp(entry, -CORRHIST_MAX, CORRHIST_MAX);
...
int adjustEvalWithCorrHist(Board& board, const int rawEval)
{
	...
	uint64_t rookPawnKey = generate_RookPawn_Hash(board);
	const int& rookPawnEntry = RookPawn_Corrhist[board.side][rookPawnKey % CORRHIST_SIZE];

	int mate_found = 49000 - 99;

	int adjust = pawnEntry + minorEntry + rookPawnEntry;
...
//}
	...
	if (!is_in_check(board) && (bestmove == Move(0, 0, 0, 0) || is_quiet(bestmove.Type)) && !(bound == HFLOWER && bestValue <= static_eval) && !(bound == HFUPPER && bestValue >= static_eval))
	{
		...
		update_RookPawn_Corrhist(board, depth, bestValue - static_eval);
	}
